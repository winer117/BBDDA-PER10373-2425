...
public class MySqlApplication {
    ...
    public static void main(String[] args) {
        ...
        try (Session session = HibernateMySqlConfig.getSessionFactory().openSession()) {
            ...
            // Primera Query
            List<Object[]> employees = employeesDao.countByGender();
            for (Object[] employee : employees) {
                log.info("Género: {}, Número de empleados: {}", employee[0], employee[1]);
            }

            // Segunda Query
            List<Object[]> bestPaidEmployee = employeesDao.bestPaidEmployee("d001");
            for (Object[] employee : bestPaidEmployee) {
                log.info("Nombre: {}, Apellido: {}, Salario: {}", employee[0], employee[1], employee[2]);
            }

            // Tercera Query
            List<Object[]> secondBestPaidEmployee = employeesDao.secondBestPaidEmployee("d001");
            for (Object[] employee : secondBestPaidEmployee) {
                log.info("Nombre: {}, Apellido: {}, Salario: {}", employee[0], employee[1], employee[2]);
            }

            // Cuarta Query
            Long countEmployees = (Long) employeesDao.employeesHiredInMonth(1);
            log.info("Número de empleados contratados en enero: {}", countEmployees);
