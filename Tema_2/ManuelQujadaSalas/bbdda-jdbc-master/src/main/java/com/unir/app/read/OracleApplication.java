...
public class OracleApplication {
    ...
    private static void selectAllEmployeesAsXml(Connection connection) throws SQLException {
        PreparedStatement selectCountries = connection.prepareStatement("SELECT  XMLELEMENT(\n" +
                "                \"empleados\",\n" +
                "                XMLFOREST(\n" +
        ...
        ResultSet employees = selectCountries.executeQuery();
        while (employees.next()) {
            log.debug("Employees as XML: {}", employees.getString("empleado_xml"));
        }
    }

    private static void selectAllManagersAsXml(Connection connection) throws SQLException {
        PreparedStatement selectCountries = connection.prepareStatement("SELECT XMLELEMENT(\"managers\",\n" +
                "           XMLAGG(\n" +
                "               XMLELEMENT(\"manager\",\n" +
        ...
        ResultSet managers = selectCountries.executeQuery();
        while (managers.next()) {
            log.debug("Managers as XML: {}", managers.getString("managers_xml"));
        }
    }
