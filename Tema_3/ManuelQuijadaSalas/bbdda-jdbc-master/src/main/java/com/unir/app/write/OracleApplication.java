...
import com.unir.model.OracleDepartment;
...
public class OracleApplication {
    ...
    public static void main(String[] args) {
        ...
        //    upsert(connection, spain);
             ...
             */
            OracleDepartment department = new OracleDepartment(300, "Data Analytics", 205, 1700);

            //el metodo intentará hacer un insert si no existe y si existe dejará un mensaje indicado ya que la tarea indica que se haga únicamente un insert
            insertDepartment(connection, department);
    ...
    public static void insertDepartment(Connection connection, OracleDepartment department) throws SQLException {
        ...
        PreparedStatement pSelect = connection.prepareStatement(selectSql);
        pSelect.setInt(1, department.getDepartmentId());
        ...
        if(rowCount >0){
            ...
            log.info("El registro con id de departamento " +  department.getDepartmentId() + " ya existe en la tabla de departamentos!!");
        }else{

            PreparedStatement pInsert = connection.prepareStatement(insertSql);
            pInsert.setInt(1, department.getDepartmentId());
            pInsert.setString(2, department.getDepartmentName());
            pInsert.setInt(3, department.getManagerId());
            pInsert.setInt(4, department.getLocationId());

            int filasInsertadas = pInsert.executeUpdate();
            log.info("Filas Insertadas: {}", filasInsertadas);
